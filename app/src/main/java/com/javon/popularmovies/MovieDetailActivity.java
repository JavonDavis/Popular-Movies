...
public class MovieDetailActivity extends AppCompatActivity implements MovieDetailFragment.OnFavoritesUpdatedListener{
    ...
    public void setupToolbar(Movie movie)
    {
        if(movie != null) {
            ...
            if (appBarLayout != null) {
                ...
                if (movie.getBackdropPath() != null && !movie.getBackdropPath().isEmpty()) {
                    ImageView backdrop = (ImageView) appBarLayout.findViewById(R.id.backdrop);
                    Picasso.with(this)
                            .load("http://image.tmdb.org/t/p/w342/" + movie.getBackdropPath())
                            .into(backdrop);
                }
                //Assuming original title will always have a value for a movie
                appBarLayout.setTitle(movie.getTitle() != null ? movie.getTitle() : movie.getOriginalTitle());
            }
        }
    }
